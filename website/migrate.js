...
migrateDB(path.join(__dirname, "../data/dicts/vino.sqlite"));

function migrateDB(newPath){
  ...
  var oldDB=new sqlite3.Database(oldPath, sqlite3.OPEN_READONLY);

  //create new database and open it:
  fs.copySync(blankPath, newPath);
  var newDB=new sqlite3.Database(newPath, sqlite3.OPEN_READWRITE);
  ...
  function migrateEntries(next){
    ...
    oldDB.each("select id, xml, title, sortkey, needs_resave from entries", function(err, row){
      newDB.run("insert into entries(id, xml, title, sortkey, needs_resave) values($id, $xml, $title, $sortkey, $needs_resave)", {
        $id: row.id,
        $xml: row.xml,
        $title: row.title,
        $sortkey: row.sortkey,
