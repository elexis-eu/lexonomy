...
Titling.render=function(div, json){
  ...
  for(var i=0; i<elements.length; i++){
    $block.find("select").append("<option "+(json.headword==elements[i] ? "selected='selected'" : "")+" value='"+elements[i]+"'>"+xema.elements[elements[i]].elementName+"</option>");
  }
  ...
  $block.find("select").append("<option value=''>(not set)</option>");
  for(var i=0; i<elements.length; i++){
    $block.find("select").append("<option "+(json.headwordSorting==elements[i] ? "selected='selected'" : "")+" value='"+elements[i]+"'>"+xema.elements[elements[i]].elementName+"</option>");
  }
  $block.find("select").on("change", function(e){Titling.headwordChanged(); Titling.change();});
  ...
  for(var i=0; i<elements.length; i++){
    $scrollbox.append("<div "+(json.headwordAnnotationsType=="simple"?"":"style='display: none'")+"><label class='radio' data-name='"+elements[i]+"'>\
          ...
          "+xema.elements[elements[i]].elementName+"</label></div>");
  }
